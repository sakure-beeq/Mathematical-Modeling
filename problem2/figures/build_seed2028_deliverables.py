"""Build the complete problem-2 figure pack for the selected seed-2028 checkpoint.

Run after evaluating train/valid/test, attachment 3, and validation robustness.
The legacy architecture ablation is explicitly identified as a different run.
"""
...
OUT = ROOT / "outputs/deliverable_seed2028"
CHARTS = OUT / "charts"
...
ABLATION_CSV = ROOT / "outputs/ablation_summary.csv"
if not ABLATION_CSV.exists():
    ABLATION_CSV = ROOT / "release/seed2028/historical_ablation_seed2026_dropout02.csv"
...
def robustness_charts(rows, baseline, pdf, titles) -> tuple[list[dict], list[dict]]:
    ...
    names = [r["position"].title() if r["position"] != "random"
             else f"Placement {int(r['seed']) - 2025}"
             for r in selected]
...
def historical_ablation(pdf, titles) -> None:
    ...
    axes[1].set_xlabel("Historical run: seed 2026, dropout 0.2", fontsize=8, labelpad=8)
    save(fig, "A1_historical_architecture_ablation", pdf, titles,
         "附录：原架构消融对照，种子 2026、dropout 0.2；并非种子 2028 的消融结果")
...
def main() -> None:
    ...
    with PdfPages(OUT / "问题2_种子2028_全部图表.pdf") as pdf:
        ...
        missing_random_summary, _ = robustness_charts(robustness_rows, robustness_base, pdf, titles)
        attachment_summary = attachment3(pdf, titles)
        historical_ablation(pdf, titles)
    shutil.copyfile(ROOT / "outputs/dropout03_lr_drop/report/seed2028_epoch_metrics.csv",
                    OUT / "seed2028_epoch_metrics.csv")
    shutil.copyfile(ABLATION_CSV,
                    OUT / "historical_ablation_seed2026_dropout02.csv")
    missing_epochs = []
    for row in history:
    ...
    manifest = {"checkpoint": str(RUN / "best.pt"), "sha256": source_hash,
                ...
                "historical_ablation_figure": "A1_historical_architecture_ablation",
    ...
    result_lines.extend(["", "## 附件 3", "",
                         ...
                         "", "## 消融结果来源", "",
                         "架构消融图和对应 CSV 是历史实验：种子 2026、dropout 0.2；"
                         "其数值不属于本次种子 2028 最终模型。", ""])
    ...
    for stem, description in titles:
        if stem.startswith("A1_"):
            continue
        lines.append(f"- [{stem}](charts/{stem}.png) · [PDF](charts/{stem}.pdf)：{description}")
    lines += ["", "## 辅助消融图", "",
              "- [A1_historical_architecture_ablation](charts/A1_historical_architecture_ablation.png)："
              "架构选择阶段的历史对照，随机种子 2026、dropout 0.2。该图不代表本次种子 2028 权重的消融实验。",
              "", "## 数值表与提交文件", "",
